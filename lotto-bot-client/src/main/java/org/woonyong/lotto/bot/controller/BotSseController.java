...
public class BotSseController {
  ...
  public BotSseController(
      final BotInstanceManager botInstanceManager, final BotClientConfig config) {
  ...
  private BotStatusResponse createBotStatusResponse() {
    ...
    return new BotStatusResponse(
        config.getMaxCapacity(), botInstanceManager.getActiveCount(), botInfos);
  }
  ...
  private List<BotStatusResponse.BotInfo> createBotInfoList(final List<BotInstance> instances) {
